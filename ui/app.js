(function () {
    ...
    function apiPatch(path, body) {
        ...
        return fetch('/api/' + path, {
            ...
            if (resp.status === 401) { clearPin(); showPinOverlay(); throw new Error('Auth required'); }
            return parseJsonResponse(resp);
        });
    }
    ...
    if (memorySearchInput) {
        memorySearchInput.addEventListener('input', function () {
            ...
            memorySearchTimer = setTimeout(function () {
                apiGet('memory/search?q=' + encodeURIComponent(query)).then(function (data) {
            ...
            try {
                ...
                setTimeout(function () {
                    if (wsTaskResolvers['pending-' + statusId]) {
                        // Events may route by task_id from server
                    }
                }, 30000);
            } catch (err) {
