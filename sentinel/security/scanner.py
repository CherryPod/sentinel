...
# Shared regex: fenced code blocks (triple-backtick) with optional language tag.
# Group 1 = language tag (may be empty), Group 2 = block content.
# Intentionally does NOT match single-backtick inline code (`code`) — those are
# a different Markdown construct (inline spans, not executable code blocks).
...
class CredentialScanner:
    ...
    _PLACEHOLDER_PREFIXES = (
        ...
        "test1234", "password", "secret12", "12345678",
    ...
    _PLACEHOLDER_SUFFIXES = (
        "examplekey", "changeme", "placeholder", "replace_me", "change_me",
    )
        ...
        for name, pattern in self._patterns:
            for match in pattern.finditer(text):
                ...
                if name in self._SECRET_ASSIGNMENT_NAMES:
                    # Extract the value portion (after = : or whitespace)
                    value_part = re.split(r"[=:\s]+", matched_text, maxsplit=1)[-1]
                    value_lower = value_part.strip("'\"").lower()
                    if (value_lower.startswith(self._PLACEHOLDER_PREFIXES)
...
class SensitivePathScanner:
    ...
    _CODE_BLOCK_SAFE: frozenset[str] = frozenset({
        ...
        ".env",          # no sensitive .env exists in the container — secrets use
                         # Podman secrets (/run/secrets/), config uses compose env vars.
                         # Worker is air-gapped and file ops are confined to /workspace/.
                         # Still flagged in shell commands and standalone path lines.
                         # If secrets ever move to .env files, remove this exemption.
    })
    ...
    def scan_output_text(self, text: str) -> ScanResult:
        ...
        for pattern in self._patterns:
            ...
            while True:
                ...
                # Shell commands inside code blocks: flag UNLESS the pattern
                # is in _CODE_BLOCK_SAFE. "cat /proc/cpuinfo" is legitimate
                # infra code; "cat .env" is not (despite .env being in
                # _CODE_BLOCK_SAFE for non-shell references like load_dotenv).
                is_shell = self._SHELL_PREFIXES.match(line)
                if in_code_block:
                    # For shell commands, only exempt patterns that are
                    # genuinely safe even in shell context (infra paths).
                    # .env is safe in code (load_dotenv) but NOT in shell
                    # (cat .env) — so exclude it from the shell exemption.
                    safe_in_shell = self._CODE_BLOCK_SAFE - {".env"}
                    if is_shell:
                        if pattern in safe_in_shell:
                            continue
                ...
                # Check 3: shell command outside code block — flag unless
                # the pattern is _CODE_BLOCK_SAFE and surrounding text is
                # Dockerfile content. Qwen sometimes outputs Containerfiles
                # without code fences, and comment lines (# ...) match
                # _SHELL_PREFIXES. Without this, /etc/passwd in a Dockerfile
                # comment triggers a false positive.
                if is_shell:
                    safe_in_shell = self._CODE_BLOCK_SAFE - {".env"}
                    if (
                        pattern in safe_in_shell
                        and CommandPatternScanner._is_dockerfile_content(text)
                    ):
                        continue
                    matches.append(ScanMatch(
                        pattern_name="sensitive_path",
                ...
                # Check 4: standalone path-only line (just whitespace + path)
                stripped = line.strip()
            ...
            for match in pattern.finditer(text):
                ...
                if in_fenced:
                    if self._is_safe_rm_in_build_context(
                        name, match.group(), block_lang, block_content,
                    ):
                        continue
