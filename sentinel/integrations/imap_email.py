...
async def search_emails(
    ...
    try:
        return await asyncio.to_thread(_imap_search_sync, config, query, max_results)
    except ImapEmailError:
        raise
...
async def read_email(
    ...
    if not config.imap_host:
        raise ImapEmailError("IMAP host not configured")
    try:
        return await asyncio.to_thread(_imap_read_sync, config, message_id, max_body_length)
    except ImapEmailError:
        raise
    except Exception as exc:
...
async def create_draft(
    ...
    if not config.imap_host:
        raise ImapEmailError("IMAP host not configured")
    try:
        return await asyncio.to_thread(_imap_create_draft_sync, config, to, subject, body)
    except ImapEmailError:
        raise
    except Exception as exc:
