...
async def _gmail_request(
    ...
    If *client* is provided it is reused (caller manages lifecycle);
...
async def search_emails(
    ...
    """
    # Use a single httpx client for the entire search to avoid per-request overhead
    async with httpx.AsyncClient(timeout=timeout) as shared_client:
        ...
        results = await asyncio.gather(*[_fetch_one(ref) for ref in messages])
        return list(results)
