...
class TestOrchestratorConstraintWiring:
    ...
    async def test_tl4_shell_constraint_validated(self):
        ...
        orch._tool_executor.execute = AsyncMock(return_value=success_data)

        with patch.object(settings, "trust_level", 4):
            result = await orch._execute_tool_call(step, context)
    ...
    async def test_constraint_result_in_step_outcome(self):
        ...
        outcome = orch._build_step_outcome(step, result, 0.1)
...
class TestProvenanceConstraintBypass:
    ...
    async def test_tl4_constrained_step_bypasses_provenance(self):
        ...
        with patch.object(settings, "trust_level", 4), \
             patch("sentinel.planner.orchestrator.is_trust_safe_for_execution", return_value=False):
            result = await orch._execute_tool_call(step, ctx)
    ...
    async def test_bypass_logged_as_constraint_gated(self):
        ...
        with patch.object(settings, "trust_level", 4), \
             patch("sentinel.planner.orchestrator.is_trust_safe_for_execution", return_value=False), \
             patch("sentinel.planner.orchestrator.logger") as mock_logger:
