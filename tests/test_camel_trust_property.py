...
class TestTrustContamination:
    ...
    async def test_trusted_input_through_qwen_becomes_untrusted(self):
        ...
        user_input = await create_tagged_data(
    ...
    async def test_deeply_nested_contamination_propagates(self):
        ...
        layer1 = await create_tagged_data(
        ...
        layer2 = await create_tagged_data(
        ...
        layer3 = await create_tagged_data(
        ...
        assert await is_trust_safe_for_execution(layer3.id) is False

    async def test_mixed_parentage_one_untrusted_contaminates(self):
        ...
        trusted_parents = []
        for i in range(5):
            trusted_parents.append(await create_tagged_data(
                content=f"trusted data {i}",
                source=DataSource.USER,
                trust_level=TrustLevel.TRUSTED,
            ))
        untrusted_parent = await create_tagged_data(
            content="qwen output",
            source=DataSource.QWEN,
            trust_level=TrustLevel.UNTRUSTED,
        )
...
class TestMissingParentDefault:
    ...
    async def test_evicted_parent_defaults_to_untrusted(self):
        ...
        store = ProvenanceStore(pool=None)
        ...
        assert await store.get_tagged_data(parent_id) is None
    ...
    async def test_nonexistent_parent_id_defaults_to_untrusted(self):
