...
class TestSignalChannelSend:
    ...
    async def test_send_timeout_logs_and_stops(self):
        """Timeout on drain() logs a warning and stops sending remaining chunks."""
        cfg = SignalConfig(max_message_length=10)
        channel = SignalChannel(cfg)
        writer = FakeSocketWriter()
        channel._writer = writer
        ...
        msg = OutgoingMessage(
            channel_id="+1234567890",
            event_type="task.completed",
            data={"response": "a" * 25},  # would be multiple chunks
        )
        with patch("sentinel.channels.signal_channel.asyncio.wait_for", side_effect=wait_for_side_effect):
            await channel.send(msg)
        # write() is called (sync, before drain), but drain timed out — only 1 chunk attempted
        assert len(writer.written) == 1
...
# ── Confirmation/Approval Formatting Tests ───────────────────────


class TestSignalFormatConfirmation:
    def test_format_confirmation_preview(self):
        """_format_response renders a confirmation gate preview."""
    ...
    def test_format_plan_approval(self):
        """_format_response renders a plan approval request."""
        from sentinel.channels.signal_channel import _format_response
        ...
        assert "Add calendar event" in result
        assert "go" in result.lower()

    def test_format_plan_approval_no_steps(self):
        """_format_response handles plan approval with no steps."""
        from sentinel.channels.signal_channel import _format_response

        data = {
            "approval_id": "plan-002",
