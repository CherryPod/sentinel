...
class TestFormatEnrichedHistory:
    ...
    def test_successful_turn_is_one_liner(self):
        """Older successful turns are tiered to one-liner — no step detail.

        Note: the most recent turn is always expanded (even on success) so
        we add a second turn to make turn 1 non-last.
        """
        planner = self._make_planner()
        history = [
            {
        ...
        # Step detail suppressed for older successful turns (turn 1)
        lines = result.split("\n")
        turn1_section = []
        in_turn1 = False
        for line in lines:
            if "Turn 1:" in line:
                in_turn1 = True
                turn1_section.append(line)
            elif "Turn 2:" in line:
                break
            elif in_turn1:
                turn1_section.append(line)
        turn1_text = "\n".join(turn1_section)
        assert "llm_task" not in turn1_text
        assert "output=150B" not in turn1_text
