...
class TestToolExecutorWasmDispatch:
    ...
    async def test_wasm_tool_dispatched_to_sidecar(self):
        ...
        tagged, exec_meta = await executor.execute("file_read", {"path": "/workspace/test.txt"})

        mock_sidecar.execute.assert_called_once()
        assert tagged.content  # TaggedData should have content
    ...
    async def test_non_wasm_tool_uses_python_handler(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            tagged, exec_meta = await executor.execute("mkdir", {"path": test_path})

        # Sidecar should NOT have been called for mkdir
        mock_sidecar.execute.assert_not_called()
        assert tagged.content  # Should still work
    ...
    async def test_no_sidecar_uses_python_handler(self):
        ...
        with tempfile.NamedTemporaryFile(mode="w", suffix=".txt", delete=False) as f:
            ...
            f.flush()
            try:
                tagged, exec_meta = await executor.execute("file_read", {"path": f.name})
                assert "test content" in tagged.content
            finally:
                os.unlink(f.name)
    ...
    async def test_sidecar_leak_logged(self):
        ...
        tagged, exec_meta = await executor.execute("file_read", {"path": "/workspace/secrets.txt"})
    ...
    async def test_sidecar_infra_failure_falls_back_to_python(self):
        ...
        with tempfile.NamedTemporaryFile(mode="w", suffix=".txt", delete=False) as f:
            ...
            try:
                ...
                assert "fallback content" in tagged.content
            finally:
                os.unlink(f.name)
    ...
    async def test_sidecar_success_no_fallback(self):
        ...
        assert "sidecar result" in tagged.content
    ...
    async def test_shell_exec_alias_resolves(self):
        ...
        with patch("sentinel.tools.executor.asyncio.create_subprocess_exec") as mock_exec:
            ...
            tagged, exec_meta = await executor.execute("shell_exec", {"command": "echo hello"})

        assert "hello" in tagged.content
