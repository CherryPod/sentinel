...
class TestSearchMemoryTool:
    ...
    async def test_search_calls_hybrid_search(self):
        ...
        with patch("sentinel.memory.search.hybrid_search", new_callable=AsyncMock, return_value=mock_results) as mock_hs:
    ...
    async def test_search_graceful_embedding_fallback(self):
        """search_memory degrades to full-text-only if embedding fails."""
        mem = MagicMock()
        mem._db = MagicMock()
        emb = AsyncMock()
        ...
        with patch("sentinel.memory.search.hybrid_search", new_callable=AsyncMock, return_value=[]) as mock_hs:
            result = await mcp._tool_manager._tools["search_memory"].fn(query="test")
...
class TestStoreMemoryTool:
    ...
    async def test_store_rejects_oversized_text(self):
        """store_memory rejects text exceeding max length."""
        mem = MagicMock()
        mcp = create_mcp_server(MagicMock(), mem, None, EventBus())
        oversized = "x" * 50_001
        result = await mcp._tool_manager._tools["store_memory"].fn(text=oversized)
        data = json.loads(result)
        assert data["status"] == "error"
        assert "too long" in data["reason"]

    async def test_store_with_embeddings(self):
        """store_memory stores chunks with embeddings when available."""
        mem = MagicMock()
        mem.store_with_embedding = AsyncMock(return_value="chunk-1")
        emb = AsyncMock()
    ...
    async def test_store_without_embeddings(self):
        ...
        mem = MagicMock()
        mem.store = AsyncMock(return_value="chunk-1")
        mcp = create_mcp_server(MagicMock(), mem, None, EventBus())
...
class TestRunTaskTool:
    ...
    async def test_run_task_rejects_oversized_input(self):
        """run_task rejects input exceeding max length."""
        orch = AsyncMock()
        mcp = create_mcp_server(orch, None, None, EventBus())
        oversized = "x" * 10_001
        result = await mcp._tool_manager._tools["run_task"].fn(request=oversized)
        data = json.loads(result)
        assert data["status"] == "error"
        assert "too long" in data["reason"]
        orch.handle_task.assert_not_called()

    async def test_run_task_passes_source_key(self):
        """run_task passes source_key='mcp:local' and task_id to orchestrator."""
        orch = AsyncMock()
        orch.handle_task = AsyncMock(return_value=TaskResult(
            status="success", plan_summary="Done",
        ))
        mcp = create_mcp_server(orch, None, None, EventBus())
        await mcp._tool_manager._tools["run_task"].fn(request="test")
        call_kwargs = orch.handle_task.call_args.kwargs
        assert call_kwargs["source_key"] == "mcp:local"
        assert call_kwargs["task_id"]  # non-empty UUID string
