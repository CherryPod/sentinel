...
Uses a minimal FastAPI app with real MemoryStore (in-memory) + mocked EmbeddingClient
...
class TestGetEndpoint:
    ...
    def test_get_existing_chunk(self, client, memory_store):
        chunk_id = _run(memory_store.store(content="Fetch me", source="test"))
...
class TestDeleteEndpoint:
    ...
    def test_delete_existing_chunk(self, client, memory_store):
        chunk_id = _run(memory_store.store(content="Delete me", source="test"))
        resp = client.delete(f"/api/memory/{chunk_id}")
        assert resp.status_code == 200
        assert resp.json()["status"] == "ok"
        assert resp.json()["deleted"] == chunk_id
        assert _run(memory_store.get(chunk_id)) is None
