...
class TestChannelRouter:
    ...
    async def test_handle_approval_granted(self):
        ...
        orch.submit_approval = AsyncMock(return_value=True)
    ...
    async def test_handle_approval_invalid(self):
        ...
        router, orch, _ = self._make_router()
        orch.approval_manager = MagicMock()
        orch.submit_approval = AsyncMock(return_value=False)
...
@patch("sentinel.planner.orchestrator.settings", MagicMock(
    ...
    planner_timeout=120, worker_timeout=480, tool_timeout=60,
    plan_execution_timeout=1500, api_task_timeout=1800,
))
class TestOrchestratorEventBusWiring:
    ...
    def _make_orchestrator(self, bus=None):
        ...
        planner = AsyncMock()
        pipeline = MagicMock()
        pipeline.scan_input = AsyncMock(return_value=MagicMock(is_clean=True))
        pipeline.scan_output = AsyncMock(return_value=MagicMock(is_clean=True, flagged_scanners=[]))
    ...
    async def test_events_emitted_on_success(self):
        ...
        orch._pipeline.process_with_qwen = AsyncMock(
            return_value=(MagicMock(id="d1", content="result", trust_level="untrusted", scan_results={}), None),
        )
