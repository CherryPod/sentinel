...
async def test_database_unavailable(mock_planner, mock_pipeline):
    """Broken session store returns clear error — no unhandled 500."""
    from unittest.mock import AsyncMock as _AsyncMock
    session_store = MagicMock()
    session_store.get_or_create = _AsyncMock(
        side_effect=Exception("connection refused")
    )
    session_store.get_lock = MagicMock(return_value=asyncio.Lock())
    ...
    assert result.status == "error"
    assert "Service temporarily unavailable" in result.reason
    mock_planner.create_plan.assert_not_called()
...
async def test_concurrent_task_isolation(mock_planner, mock_pipeline):
    ...
    # Different sessions
    session_a = await session_store.get("api:user_a")
    session_b = await session_store.get("api:user_b")
