...
in-memory SQLite.  freezegun is used via context manager for
...
class TestRoutineCronFiresOnSchedule:
    ...
    async def test_cron_fires_on_schedule(self, engine, store, mock_orchestrator, db):
        ...
        rows = db.execute("SELECT status FROM routine_executions").fetchall()
        assert len(rows) >= 1
...
class TestRoutineMaxConcurrent:
    ...
    async def test_max_concurrent_limits_spawns(self, store, bus, db):
        hold = asyncio.Event()
        ...
        eng = RoutineEngine(
            ...
            db=db, tick_interval=1, max_concurrent=2, execution_timeout=30,
        )
...
class TestRoutineTimeout:
    ...
    async def test_timeout_records_status(self, store, bus, db):
        ...
        eng = RoutineEngine(
            ...
            db=db, tick_interval=1, max_concurrent=3, execution_timeout=1,
        )
...
class TestRoutineErrorHandling:
    ...
    async def test_error_recorded_and_engine_continues(self, store, bus, db):
        call_count = 0
        ...
        history = eng.get_execution_history(r1.routine_id)
        ...
        history2 = eng.get_execution_history(r2.routine_id)
...
class TestRoutineDedup:
    ...
    async def test_rapid_events_deduped_by_cooldown(self, store, bus, db):
        orch = AsyncMock()
        orch.handle_task = AsyncMock(return_value=TaskResult(
...
class TestRoutinePersistenceAcrossRestart:
    """routine_persistence_across_restart — routine survives store re-creation."""

    def test_routine_persists_in_sqlite(self, db):
        store1 = RoutineStore(db=db)
        r = store1.create(
        ...
        store2 = RoutineStore(db=db)
        loaded = store2.get(routine_id)
...
class TestRoutineDisableToggle:
    ...
    async def test_disabled_routine_skipped(self, engine, store, mock_orchestrator):
        ...
        # Disable the routine
        store.update(r.routine_id, enabled=False)
...
class TestRoutineInflightOnRestart:
    ...
    async def test_stale_executions_cleaned_on_start(self, store, bus, db):
        ...
        # Manually insert stale "running" executions (simulates crash)
        db.execute(
            """INSERT INTO routine_executions
               (execution_id, routine_id, user_id, triggered_by, started_at, status)
               VALUES ('stale-exec-1', ?, 'default', 'scheduler',
                       '2026-02-17T08:00:00.000Z', 'running')""",
            (r.routine_id,),
        )
        db.execute(
            """INSERT INTO routine_executions
               (execution_id, routine_id, user_id, triggered_by, started_at, status)
               VALUES ('stale-exec-2', ?, 'default', 'manual',
                       '2026-02-17T08:30:00.000Z', 'running')""",
            (r.routine_id,),
        )
        db.commit()
        ...
        rows = db.execute(
            "SELECT execution_id, status, error FROM routine_executions "
            "WHERE execution_id IN ('stale-exec-1', 'stale-exec-2') "
            "ORDER BY execution_id"
        ).fetchall()
        assert len(rows) == 2
        for row in rows:
            assert row[1] == "interrupted"
            assert "Engine restarted" in row[2]
