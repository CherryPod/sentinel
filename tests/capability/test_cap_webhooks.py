...
@pytest.fixture
async def registry():
    return WebhookRegistry(pool=None)
...
class TestWebhookRegistry:
    async def test_register_and_get(self, registry, webhook_secret):
        config = await registry.register(name="Test Hook", secret=webhook_secret)
        assert config.webhook_id
        assert config.name == "Test Hook"

        fetched = await registry.get(config.webhook_id)
        assert fetched is not None
        assert fetched.secret == webhook_secret

    async def test_get_unknown(self, registry):
        assert await registry.get("nonexistent") is None

    async def test_delete(self, registry, webhook_secret):
        config = await registry.register(name="Deletable", secret=webhook_secret)
        assert await registry.delete(config.webhook_id) is True
        assert await registry.get(config.webhook_id) is None

    async def test_list(self, registry, webhook_secret):
        await registry.register(name="Hook 1", secret=webhook_secret)
        await registry.register(name="Hook 2", secret=webhook_secret)
        assert len(await registry.list()) == 2

    async def test_inmemory_fallback(self, webhook_secret):
        reg = WebhookRegistry(pool=None)
        config = await reg.register(name="In-Memory", secret=webhook_secret)
        assert await reg.get(config.webhook_id) is not None
...
class TestWebhookReceiveValidPayload:
    ...
    async def test_valid_payload_publishes_event(self, registry, bus, webhook_secret):
        config = await registry.register(name="Valid Test", secret=webhook_secret)
    ...
    async def test_endpoint_valid_payload(self, webhook_secret):
        """Full endpoint test via httpx."""
        import httpx
        from sentinel.api.app import app

        reg = WebhookRegistry(pool=None)
        config = await reg.register(name="Endpoint Test", secret=webhook_secret)
        ...
        with (
            patch("sentinel.api.routes.webhooks._webhook_registry", reg),
            patch("sentinel.api.routes.webhooks._event_bus", event_bus),
            patch("sentinel.api.routes.webhooks._webhook_rate_limiter", rate_limiter),
            patch("sentinel.api.routes.webhooks._idempotency_cache", {}),
        ):
...
class TestWebhookInvalidSignature:
    ...
    async def test_invalid_sig_rejected(self, webhook_secret):
        import httpx
        from sentinel.api.app import app

        reg = WebhookRegistry(pool=None)
        config = await reg.register(name="Sig Test", secret=webhook_secret)
        ...
        with (
            patch("sentinel.api.routes.webhooks._webhook_registry", reg),
            patch("sentinel.api.routes.webhooks._event_bus", EventBus()),
            patch("sentinel.api.routes.webhooks._webhook_rate_limiter", RateLimiter()),
            patch("sentinel.api.routes.webhooks._idempotency_cache", {}),
        ):
            transport = httpx.ASGITransport(app=app)
            async with httpx.AsyncClient(transport=transport, base_url="http://test") as client:
...
class TestWebhookUnknownId:
    ...
    async def test_unknown_webhook_404(self):
        import httpx
        from sentinel.api.app import app

        with (
            patch("sentinel.api.routes.webhooks._webhook_registry", WebhookRegistry(pool=None)),
            patch("sentinel.api.routes.webhooks._event_bus", EventBus()),
            patch("sentinel.api.routes.webhooks._webhook_rate_limiter", RateLimiter()),
            patch("sentinel.api.routes.webhooks._idempotency_cache", {}),
        ):
...
class TestWebhookTriggersRoutine:
    ...
    async def test_event_routine_fires(self):
        bus = EventBus()
        reg = WebhookRegistry(pool=None)
        config = await reg.register(name="Trigger Test", secret="x" * 16)
...
class TestWebhookRateLimiting:
    ...
    async def test_rate_limit_enforced(self, webhook_secret):
        import httpx
        from sentinel.api.app import app

        reg = WebhookRegistry(pool=None)
        config = await reg.register(name="Rate Test", secret=webhook_secret)
        rl = RateLimiter(max_per_minute=3)
        ...
        with (
            patch("sentinel.api.routes.webhooks._webhook_registry", reg),
            patch("sentinel.api.routes.webhooks._event_bus", EventBus()),
            patch("sentinel.api.routes.webhooks._webhook_rate_limiter", rl),
            patch("sentinel.api.routes.webhooks._idempotency_cache", {}),
        ):
            transport = httpx.ASGITransport(app=app)
            async with httpx.AsyncClient(transport=transport, base_url="http://test") as client:
...
class TestWebhookPayloadUntrusted:
    ...
    async def test_prompt_triggers_as_webhook_source(self, webhook_secret):
        """Verify that webhook payloads with prompt field route as 'webhook' source."""
        import httpx
        from sentinel.api.app import app

        reg = WebhookRegistry(pool=None)
        config = await reg.register(name="Untrusted Test", secret=webhook_secret)
        ...
        with (
            patch("sentinel.api.routes.webhooks._webhook_registry", reg),
            patch("sentinel.api.routes.webhooks._event_bus", bus),
            patch("sentinel.api.routes.webhooks._orchestrator", mock_orch),
            patch("sentinel.api.routes.webhooks._webhook_rate_limiter", RateLimiter()),
            patch("sentinel.api.routes.webhooks._idempotency_cache", {}),
        ):
            transport = httpx.ASGITransport(app=app)
...
class TestWebhookIdempotency:
    ...
    async def test_duplicate_rejected(self, webhook_secret):
        import httpx
        from sentinel.api.app import app

        reg = WebhookRegistry(pool=None)
        config = await reg.register(name="Idemp Test", secret=webhook_secret)
        ...
        with (
            ...
            patch("sentinel.api.routes.webhooks._event_bus", EventBus()),
            patch("sentinel.api.routes.webhooks._webhook_rate_limiter", RateLimiter()),
            patch("sentinel.api.routes.webhooks._idempotency_cache", cache),
        ):
            transport = httpx.ASGITransport(app=app)
            async with httpx.AsyncClient(transport=transport, base_url="http://test") as client:
                headers = {
...
class TestWebhookReplayAttack:
    ...
    async def test_old_timestamp_rejected(self, webhook_secret):
        import httpx
        from sentinel.api.app import app

        reg = WebhookRegistry(pool=None)
        config = await reg.register(name="Replay Test", secret=webhook_secret)
