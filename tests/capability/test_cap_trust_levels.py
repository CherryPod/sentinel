...
class TestProvenanceTrustTracking:
    ...
    @pytest.mark.capability
    def test_tl2_provenance_on_read_files(self):
        ...
        assert is_trust_safe_for_execution(file_data.id) is True

    @pytest.mark.capability
    def test_tl2_qwen_written_file_fed_back_to_planner(self):
        ...
        record_file_write("/workspace/payload.sh", qwen_output.id)

        # Step 3: File read back — creates new TaggedData with parent chain
        read_data = create_tagged_data(
            content="malicious payload",
            source=DataSource.FILE,
        ...
        assert is_trust_safe_for_execution(read_data.id) is False

    @pytest.mark.capability
    def test_trust_level_downgrade_takes_effect_immediately(self):
        ...
        """
        trusted = create_tagged_data(
            content="safe user input",
            source=DataSource.USER,
            trust_level=TrustLevel.TRUSTED,
        )
        untrusted = create_tagged_data(
        ...
        assert is_trust_safe_for_execution(trusted.id) is True
...
class TestSafeToolDispatch:
    ...
    async def test_safe_tool_result_is_trusted(self, mock_planner, mock_pipeline):
        ...
        tagged = get_tagged_data(data_id)
    ...
    def test_planner_receives_safe_tool_descriptions(self, mock_planner, mock_pipeline):
        ...
        safe_tools = orch._get_safe_tool_descriptions()
        ...
        safe_tools2 = orch2._get_safe_tool_descriptions()
        ...
        safe_tools3 = orch2._get_safe_tool_descriptions()
...
class TestFileWriteTL3Provenance:
    ...
    @pytest.mark.capability
    def test_tl3_written_file_inherits_untrusted(self):
