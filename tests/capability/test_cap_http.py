...
def _mock_engine(policy: dict | None = None) -> MagicMock:
    ...
    default_domains = ["api.search.brave.com", "*.googleapis.com"]
    policy_dict = policy or {
        "network": {
            "http_tool_allowed_domains": default_domains,
        },
    }
    engine._policy = policy_dict
    engine.get_http_allowlist.return_value = (
        policy_dict.get("network", {}).get("http_tool_allowed_domains", [])
    )
    return engine
...
async def test_http_get_success():
    ...
    result, _ = await executor.execute("http_fetch", {
...
async def test_http_response_enters_pipeline_as_untrusted():
    ...
    assert await is_trust_safe_for_execution(result.id) is False
