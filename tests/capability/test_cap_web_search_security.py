...
def _make_web_search_result(**overrides) -> TaggedData:
    ...
    return create_tagged_data(**defaults)
...
def test_web_search_injection_in_results_spotlighted():
...
@pytest.mark.capability
def test_web_search_provenance_trust_gate():
    """is_trust_safe_for_execution() returns False for web search data."""
    tagged = _make_web_search_result()

    # Web search data should be blocked by trust gate
    assert is_trust_safe_for_execution(tagged.id) is False
...
@pytest.mark.capability
def test_web_search_provenance_chain_integrity():
    """WEB->QWEN chain: child inherits UNTRUSTED from parent."""
    # Web search result (UNTRUSTED)
    web_data = _make_web_search_result()
    ...
    assert is_trust_safe_for_execution(qwen_output.id) is False
...
def test_web_search_qwen_output_fed_back():
    ...
    assert is_trust_safe_for_execution(web_result.id) is False
    assert is_trust_safe_for_execution(qwen_step1.id) is False
    assert is_trust_safe_for_execution(qwen_step2.id) is False
