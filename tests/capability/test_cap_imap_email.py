...
@pytest.mark.capability
def test_build_imap_search_strips_quotes():
    """Quotes in query values are stripped to prevent IMAP command injection."""
    from sentinel.integrations.imap_email import _build_imap_search
    result = _build_imap_search('from:alice"@evil.com')
    assert 'FROM "alice@evil.com"' in result
    result2 = _build_imap_search('hello "world')
    assert 'TEXT "hello world"' in result2
...
async def test_imap_search_most_recent_first():
    ...
    with patch("sentinel.core.config.settings", mock_settings):
        with patch("sentinel.integrations.imap_email._imap_connect", return_value=conn):
            executor = ToolExecutor(policy_engine=_mock_engine())
            result, _ = await executor.execute("email_search", {"query": "test"})
...
@pytest.mark.capability
@pytest.mark.asyncio
async def test_imap_search_empty_query_lists_all():
    """Empty query string falls back to '*' (list all recent emails)."""
    uids = [b"1", b"2"]
    conn = _mock_imap_conn(search_uids=uids)
    ...
    with patch("sentinel.core.config.settings", mock_settings):
        with patch("sentinel.integrations.imap_email._imap_connect", return_value=conn):
            executor = ToolExecutor(policy_engine=_mock_engine())
            result, _ = await executor.execute("email_search", {"query": ""})
...
async def test_imap_read_email():
    ...
    with patch("sentinel.core.config.settings", mock_settings):
        with patch("sentinel.integrations.imap_email._imap_connect", return_value=conn):
            executor = ToolExecutor(policy_engine=_mock_engine())
            result, _ = await executor.execute("email_read", {"message_id": "101"})
...
@pytest.mark.capability
@pytest.mark.asyncio
async def test_imap_send_missing_recipient():
    """Missing recipient → ToolError."""
    mock_settings = _imap_settings()
    with patch("sentinel.core.config.settings", mock_settings):
        executor = ToolExecutor(policy_engine=_mock_engine())
...
async def test_imap_create_draft():
    ...
    with patch("sentinel.core.config.settings", mock_settings):
        with patch("sentinel.integrations.imap_email._imap_connect", return_value=conn):
            executor = ToolExecutor(policy_engine=_mock_engine())
            result, _ = await executor.execute("email_draft", {
                "recipient": "recipient@example.com",
...
@pytest.mark.capability
@pytest.mark.asyncio
async def test_imap_draft_missing_recipient():
    """Draft with missing recipient → ToolError."""
    mock_settings = _imap_settings()
    with patch("sentinel.core.config.settings", mock_settings):
        executor = ToolExecutor(policy_engine=_mock_engine())
        with pytest.raises(ToolError, match="No recipient"):
...
async def test_imap_host_not_configured():
    """IMAP host empty → ToolError about unconfigured email."""
    mock_settings = _imap_settings(imap_host="")
    with patch("sentinel.core.config.settings", mock_settings):
        executor = ToolExecutor(policy_engine=_mock_engine())
        with pytest.raises(ToolError, match="Email not configured"):
            await executor.execute("email_search", {"query": "test"})
