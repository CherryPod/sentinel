...
@pytest.fixture
def routine_store():
    """Create a RoutineStore with SQLite backend.
...
def client_with_store(routine_store):
    ...
    with patch("sentinel.api.app._pin_verifier", None), \
         patch("sentinel.api.app._routine_store", routine_store), \
         patch("sentinel.api.app._routine_engine", None):
...
class TestRoutineToggle:
    ...
    @pytest.mark.capability
    def test_routine_toggle_enabled(self, client_with_store):
...
class TestRoutineDelete:
    ...
    @pytest.mark.capability
    def test_routine_delete(self, client_with_store):
        ...
        resp = client.delete(f"/api/routine/{routine.routine_id}")
        assert resp.status_code == 200
...
class TestRoutineExecutionHistory:
    ...
    @pytest.mark.capability
    def test_routine_execution_history(self, client_with_store):
...
class TestRoutineManualTrigger:
    ...
    @pytest.mark.capability
    def test_routine_manual_trigger_engine_disabled(self, client_with_store):
        ...
        resp = client.post(f"/api/routine/{routine.routine_id}/run")
...
class TestRoutineRateLimit:
    ...
    def test_routine_rate_limit(self):
        ...
        try:
            with patch("sentinel.api.app._routine_store", store), \
                 patch("sentinel.api.app._routine_engine", None):
