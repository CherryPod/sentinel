...
async def test_sidecar_tool_dispatch_file_read():
    ...
    result, _ = await executor.execute("file_read", {"path": "/workspace/test.txt"})
...
async def test_sidecar_tool_dispatch_shell():
    ...
    result, _ = await executor.execute("shell_exec", {"command": "echo hello"})
...
async def test_sidecar_recovery_after_crash():
    ...
    with patch("asyncio.open_unix_connection", side_effect=mock_connect):
        with patch.object(client, "start_sidecar", new_callable=AsyncMock):
            tagged, _ = await executor.execute("file_read", {"path": "/workspace/test.txt"})
...
async def test_sidecar_no_state_bleed_between_dispatches():
    ...
    result_1, _ = await executor.execute("file_read", {"path": "/workspace/a.txt"})
    result_2, _ = await executor.execute("file_read", {"path": "/workspace/b.txt"})
