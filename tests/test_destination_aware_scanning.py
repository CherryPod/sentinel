...
class TestScanOutputDestination:
    ...
    async def test_execution_destination_runs_cmd_scanner(self, pipeline):
        """CommandPatternScanner runs and blocks when destination=EXECUTION."""
        self._setup_scanners(pipeline)
        result = await pipeline.scan_output("chmod u+s /usr/bin/foo", OutputDestination.EXECUTION)
        assert not result.is_clean
        assert "command_pattern_scanner" in result.violations

    async def test_display_destination_skips_cmd_scanner(self, pipeline):
        """CommandPatternScanner skipped when destination=DISPLAY."""
        self._setup_scanners(pipeline)
        result = await pipeline.scan_output("chmod u+s /usr/bin/foo", OutputDestination.DISPLAY)
        assert result.is_clean
        pipeline._cmd_scanner.scan_output_text.assert_not_called()

    async def test_display_destination_still_runs_credential_scanner(self, pipeline):
        ...
        result = await pipeline.scan_output("AKIAIOSFODNN7EXAMPLE", OutputDestination.DISPLAY)
        assert not result.is_clean
        assert "credential_scanner" in result.violations

    async def test_default_destination_is_execution(self, pipeline):
        """Default destination is EXECUTION when not specified (backward compat)."""
        self._setup_scanners(pipeline)
        result = await pipeline.scan_output("chmod u+s /usr/bin/foo")
...
class TestProcessWithQwenDestination:
    ...
    def _make_pipeline(self):
        ...
        pipeline._worker.generate = AsyncMock(return_value=("response text", None))
    ...
    async def test_destination_threaded_to_scan_output(self):
        ...
        with patch.object(pipeline, "scan_input", new_callable=AsyncMock), \
             patch.object(pipeline, "scan_output", new_callable=AsyncMock, return_value=PipelineScanResult()) as mock_scan, \
...
class TestDestinationPrecomputation:
    ...
    def test_single_llm_task_is_display(self):
        ...
        from sentinel.planner.builders import compute_execution_vars, get_destination
        ...
        assert get_destination(plan.steps[0], execution_vars) == OutputDestination.DISPLAY
    ...
    def test_llm_task_chained_to_tool_call_is_execution(self):
        ...
        execution_vars = compute_execution_vars(plan)
        assert get_destination(plan.steps[0], execution_vars) == OutputDestination.EXECUTION
        # tool_call steps always default to EXECUTION (doesn't matter, they don't scan)
        assert get_destination(plan.steps[1], execution_vars) == OutputDestination.EXECUTION
    ...
    def test_llm_task_with_output_var_but_no_consumer_is_display(self):
        ...
        execution_vars = compute_execution_vars(plan)
        assert get_destination(plan.steps[0], execution_vars) == OutputDestination.DISPLAY
        assert get_destination(plan.steps[1], execution_vars) == OutputDestination.DISPLAY
...
class TestDestinationAwareScanningE2E:
    ...
    async def test_display_output_with_cmd_pattern_passes(self, pipeline):
        ...
        result = await pipeline.scan_output(text, destination=OutputDestination.DISPLAY)
        assert result.is_clean

    async def test_execution_output_with_cmd_pattern_blocks(self, pipeline):
        """Same content blocks on EXECUTION path."""
        text = "To set the setuid bit, use:\n```bash\nchmod u+s /usr/local/bin/myapp\n```"
        result = await pipeline.scan_output(text, destination=OutputDestination.EXECUTION)
        assert not result.is_clean
        assert "command_pattern_scanner" in result.violations

    async def test_display_still_blocks_credentials(self, pipeline):
    ...
    async def test_display_cmd_scanner_result_present_but_clean(self, pipeline):
    ...
    async def test_log_contains_destination_field(self, pipeline):
        """Verify destination appears in scan_output log entry."""
        import logging
        with patch.object(logging.getLogger("sentinel.audit"), "info") as mock_log:
            await pipeline.scan_output("hello world", destination=OutputDestination.DISPLAY)
