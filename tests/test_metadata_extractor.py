...
class TestExtractStderrPreview:
    ...
    def test_default_max_lines_truncates_to_ten(self):
        # 15-line input should be truncated to 10 lines using default max_lines
        from sentinel.analysis.metadata_extractor import extract_stderr_preview
        stderr = "\n".join(f"line {i}" for i in range(15))
        result = extract_stderr_preview(stderr)
        assert result.count("\n") == 9  # 10 lines = 9 newlines
        assert result.startswith("line 0")
        assert "line 10" not in result

    def test_default_max_chars_truncates_to_1500(self):
        # Input longer than 1500 chars should be truncated using default max_chars
        from sentinel.analysis.metadata_extractor import extract_stderr_preview
        stderr = "x" * 3000
        result = extract_stderr_preview(stderr)
