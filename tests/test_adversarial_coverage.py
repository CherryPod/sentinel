...
class TestAdversarialInputScanning:
    ...
    async def test_adversarial_input_scanned(self, real_pipeline, prompt, category, expected_catch):
        ...
        result = await real_pipeline.scan_input(prompt)
...
class TestLegitimatePromptsFalsePositive:
    ...
    async def test_legitimate_prompt_passes_clean(self, real_pipeline, prompt):
...
class TestEdgeCaseHandling:
    ...
    async def test_edge_case_no_crash(self, real_pipeline, prompt, should_catch, expected):
...
class TestEncodingEvasion:
    ...
    async def test_cyrillic_homoglyph_path(self, real_pipeline):
        ...
        result = await real_pipeline.scan_input(attack)
    ...
    async def test_rtl_override_evasion(self, real_pipeline):
...
class TestScannerAttribution:
    ...
    async def test_correct_scanner_catches_attack(self, real_pipeline, attack, expected_scanner):
...
class TestIndirectInjection:
    ...
    async def test_injection_in_document(self, real_pipeline):
        ...
        result = await real_pipeline.scan_input(document)
    ...
    async def test_injection_in_log_entry(self, real_pipeline):
    ...
    async def test_injection_in_json(self, real_pipeline):
        ...
        result = await real_pipeline.scan_input(json_data)
        assert not result.is_clean, "Credential in JSON not caught"
        assert "credential_scanner" in result.violations

    async def test_injection_in_email_thread(self, real_pipeline):
    ...
    async def test_injection_in_code_review(self, real_pipeline):
        ...
        result = await real_pipeline.scan_input(code_review)
        assert not result.is_clean, "Command in code comment not caught"

    async def test_injection_in_csv(self, real_pipeline):
        ...
        result = await real_pipeline.scan_input(csv_data)
        assert not result.is_clean, "Path reference in CSV not caught"

    async def test_credential_with_zero_width_spaces(self, real_pipeline):
        ...
        result = await real_pipeline.scan_input(text)
