...
class TestContainerSecurity:
    ...
    def test_sentinel_restart_policy(self, sentinel_service):
        """sentinel should have restart: always for reliability."""
        assert sentinel_service.get("restart") == "always"

    def test_ollama_restart_policy(self, ollama_service):
        """sentinel-ollama should have restart: always for reliability."""
        assert ollama_service.get("restart") == "always"
...
class TestOllamaImageSecurity:
    ...
    def test_ollama_image_pinned_by_digest(self, ollama_service):
        """Ollama image must be pinned by sha256 digest."""
        image = ollama_service.get("image", "")
        assert "@sha256:" in image, (
            f"Ollama image '{image}' must be pinned by digest (@sha256:...) "
            "to prevent supply-chain tag hijacking"
        )
